...
function getFen(time,time2){
	var long1=parseInt(time.substr(0,4)*365)+parseInt(time.substr(5,1)*30)+parseInt(time.substr(-1,2).replace("/",""));
	var long2=parseInt(time.substr(0,4)*365)+parseInt(time2.substr(5,1)*30)+parseInt(time2.substr(-1,2).replace("/",""));

	return Math.abs(long1%365-long2%365);
};
...
function setPoint(datas){
	...
	for(var i=0;i<d.length-1;i++){
		...
		var div='<span class="axisTime" style="left:'+(d[i].left-10)+'px;cursor:pointer" onClick="axisPointClick(this)" id="'+i+'">'+d[i].time+'</span>';
		$(".axis").append(div);
	}
	var i=d.length-1;
	var div='<span class="axisTime" style="left:'+(d[i].left-10)+'px;cursor:pointer" onClick="axisPointClick(this)" id="'+i+'">'+d[i].time+'</span>';
	...
	var left=getFen(time,time2)+d[d.length-1].left;
